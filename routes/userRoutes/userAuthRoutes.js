...
router.route('/add').post(async (req, res) => {
  ...
  console.log("-------Received a new User Add Request------");
  let unique = true;
  let not_unique = '';
  let user_emailcheck = await User.findOne({ email: email });
  ...
  let user_idcheck = await User.findOne({ userId: userId });
  if (user_idcheck) {
    unique = false;
    not_unique = 'ID';
  }
    ...
    try {  
      ...
      newUser
        ...
        .then((response) => {
          ...
          console.log("user Added",loginData);
          res.json(loginData);
        })
        .catch((err) => {
          console.log(err);
